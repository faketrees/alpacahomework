...
describe Dessert do
  let(:chef) { double("chef", name: "Timmy") }
  let(:cookie) { Dessert.new("cookie", 75, chef) }  
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect {Dessert.new("brownie", "two", chef)}.to raise_error(ArgumentError)
    end
  end
  ...
  describe "#mix!" do
    it "shuffles the ingredient array" do
      ingredients = ["milk", "flour", "sugar", "chocolate", "eggs", "butter", "water", "oil"]
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Chef Timmy the Great Baker")
      expect(cookie.serve).to eq("Chef Timmy the Great Baker has made 75 cookies!")
    end
  end
